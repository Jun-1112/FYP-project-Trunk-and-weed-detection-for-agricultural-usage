...
2. The problem is not that annotation is difficult. however, it is that annotating under these conditions would actively damage the model. Any polygon drawn around a weed patch would inevitably enclose adjacent fronds, leaf litter, and trunk base within the same boundary. Training on that ground truth teaches the model that fronds and leaves are weeds, producing a detector that fires on palm foliage and trunk bases in which precisely the objects the system exists to avoid. The bad ground truth here is worse than no ground truth.
